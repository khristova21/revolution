...
def upright_posture_bonus(
    env: ManagerBasedRLEnv, threshold: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("agent")
) -> torch.Tensor:
...
def move_to_target_bonus(
    ...
    asset_cfg: SceneEntityCfg = SceneEntityCfg("agent"),
) -> torch.Tensor:
...
class progress_reward(ManagerTermBase):
    ...
    def reset(self, env_ids: torch.Tensor):
        ...
        asset: Articulation = self._env.scene["agent"]
...
class joint_pos_limits_penalty_ratio(ManagerTermBase):
    ...
    def __init__(self, env: ManagerBasedRLEnv, cfg: RewardTermCfg):
        ...
        asset_cfg = cfg.params.get("asset_cfg", SceneEntityCfg("agent"))
        # extract the used quantities (to enable type-hinting)
        asset: Articulation = env.scene[asset_cfg.name]
...
class power_consumption(ManagerTermBase):
    ...
    def __call__(
        self, env: ManagerBasedRLEnv, gear_ratio: dict[str, float], asset_cfg: SceneEntityCfg = SceneEntityCfg("agent")
    ) -> torch.Tensor:
        # extract the used quantities (to enable type-hinting)
        asset: Articulation = env.scene[asset_cfg.name]
